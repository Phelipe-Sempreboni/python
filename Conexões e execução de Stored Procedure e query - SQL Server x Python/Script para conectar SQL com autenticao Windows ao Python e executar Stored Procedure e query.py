#Exemplo de conexão do Python com o SQL Server com autenticação Windows, sem necessidade de senha e execução de storede procedure e query.
...
#Nota2: Por ser login no SQL Server com (Autenticação Windows), a linha de código 27 ("Trusted_Connection=no;") necessita estar como "yes", indicando o login por esse tipo de autenticação Windows.
...
def conectar():
    try:
        conexao = pyodbc.connect(
            ...
            Trusted_Connection='yes' #Este campo em (no) refere-se ao tipo de autentição, neste caso, autenticação SQL Server. Se quiser fazer com autenticação Windows, retire o campo (pwd) que seria a senha, e mude este campo para (yes).
        )
