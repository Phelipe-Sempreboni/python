...
if __name__ == '__main__':
    ...
    for i in range (num_intervals):
        ...
        if (total_regs_intervalo>0):
            cabecalho = ['instalacao' , 'status_instalacao' , 'unidade_leitura' ,'endereco_completo', 'numero_fases', 
                     'classe_subclasse' , 'coordenada_x' ,'coordenada_y', 'cnae' , 'area_risco', 'regional', 
                     'localidade', 'bairro' , 'consumo_faturado', 'parceiro_nome', 'localidade_texto','regional_texto']
            ...
            if (i == 0):
                data_f.to_csv('elgb.txt', sep = ';', index=False) # Primeiro arquivo gera com cebecalho.
            else:
                data_f.to_csv('elgb.txt', mode = 'a', sep = ';', index=False, header=False) # Depois so acrescenta dados no arquivo
