# Exemplo de conexão do Python com o SQL Server com autenticação do SQL Server, com necessidade de senha e de uma query para atualização de registros da tabela.
...
# Nota: Neste caso, você pode trocar o comando de (UPDATE) e os parametros para o que preferir. Irei deixar o exemplo abaixo onde:
...
def query():
    ...
    try:
        cursor = conectar()
        cursor.execute("""
        UPDATE [LPUS_ENERGIA].[clientes].[tbl_cadastro_clientes]
        SET T_NOMECLIENTE = 'Ronaldo Rogerio'
        WHERE N_IDCLIENTE = 1
        """
        )
        cursor.commit()
    except:
        print('Não foi possivel atualizar os registros da tabela.')
    else:
        print('Registros atualizados com sucesso da tabela.')
