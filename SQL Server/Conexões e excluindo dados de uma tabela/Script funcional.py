# Exemplo de conexão do Python com o SQL Server com autenticação do SQL Server, com necessidade de senha e de uma query para excluir dados de uma tabela.
...
#Nota3: Sobre o (TRUNCATE TABLE):
#Cuidado com o comando (TRUNCATE TABLE), pois, ele não contém clausula (WHERE), igual um comando (DELETE FROM WHERE), e o comando (TRUNCATE TABLE) tem um (COMMIT) explicito, então, uma vez excluídos os dados, não será possível recuperação, pois, diferente do comando (DELETE), o (TRUNCATE) não possuí arquivos de logs de transação para possível recuperação dos dados.

#Nota4: Caso queira testar este comando Python com o a consulta (TRUNCATE TABLE) que foi deixada como exemplo, irei deixar abaixo do comando Python, um script para ser executado no SQL Server que irá gerar essa infos.
...
# Nota: Neste caso, você pode trocar o comando de (TRUNCATE TABLE) e os parametros para o que preferir. Irei deixar o exemplo abaixo onde:
...
def query():
    ...
    try:
        cursor = conectar()
        cursor.execute("TRUNCATE TABLE [LPUS_ENERGIA].[clientes].[tbl_cadastro_clientes]")
        cursor.commit()
    except:
        print('Não foi possivel excluir os dados da tabela.')
    else:
        print('Dados excluídos com sucesso da tabela.')
